...
def get_stations():
    df = pd.read_parquet(
        STATIONS_PATH, columns=["station_code", "station_name", "latitude", "longitude", "elevation_m", "end_date"]
    )
...
def get_station_readings(station_code: str):
    ...
    try:
        measured_at = datetime.fromtimestamp(matches[0].stat().st_mtime).strftime("%Y-%m-%d %H:%M:%S")
